  ...
  const sendChat = async () => {
    ...
    try {
      ...
      const res = await fetch(`${base}/ask`, {
        ...
        body: JSON.stringify({ question: q, listings }),
      });
      if (!res.ok) throw new Error(`API error ${res.status}`);
      const data = await res.json();
  ...
  return (
    ...
    >
      ...
      <div className="flex h-[calc(100vh-61px)]">
        ...
        {/* Chat panel */}
        <div className="w-80 flex-shrink-0 border-l border-stone-800 flex flex-col bg-stone-900">
          {/* Header */}
          <div className="px-4 py-3 border-b border-stone-800 flex-shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-5 h-5 bg-amber-400 rounded flex items-center justify-center flex-shrink-0">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#1c1917" strokeWidth="2.5"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
              </div>
              <span className="text-xs font-semibold text-stone-300">Ask about listings</span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
            {chatMessages.length === 0 && (
              <div className="text-stone-600 text-xs text-center mt-4">
                Ask anything about the current listings — price ranges, neighborhood comparisons, best value, and more.
              </div>
            )}
            ...
            {chatLoading && (
              <div className="flex justify-start">
                <div className="bg-stone-800 border border-stone-700 rounded-xl px-3 py-2 text-xs text-stone-500">
                  Thinking…
                </div>
              </div>
            )}
          ...
          <div className="p-3 border-t border-stone-800 flex-shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-stone-800 border border-stone-700 rounded-lg px-3 py-2 text-xs text-stone-200 placeholder-stone-600 focus:outline-none focus:border-amber-400/60 min-w-0"
              />
              <button
                onClick={sendChat}
                disabled={chatLoading || !chatInput.trim()}
                className="bg-amber-400 text-stone-900 rounded-lg px-3 py-2 flex-shrink-0 hover:bg-amber-300 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22,2 15,22 11,13 2,9"/></svg>
              </button>
            </div>
          </div>
        </div>
